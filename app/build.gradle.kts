...
android {
  ...
  defaultConfig {
    applicationId = "com.aistudio.merathikaana.ranchi"
    minSdk = 24
    targetSdk = 36
    val gitHubRunNumber = System.getenv("GITHUB_RUN_NUMBER")?.toIntOrNull()
    val envVersionCode = System.getenv("VERSION_CODE")?.toIntOrNull()
    versionCode = envVersionCode ?: if (gitHubRunNumber != null) (100 + gitHubRunNumber) else 2
    versionName = "1.1"
...
dependencies {
  ...
  // Google Sign-In via Credential Manager (removed: authentication is handled directly by web app)
